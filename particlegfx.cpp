		...
		if(ff->isEnabled())
		{
			...
			for(int j = -1; j < 2; j++)
			{
				...
				if(distbrightness > 0)
				{
					...
					newr = r * distbrightness / 100. * ff->getRedBrightness() * _brightness / 100.; // * _brightness;
					newg = g * distbrightness / 100. * ff->getGreenBrightness() * _brightness / 100.; // * _brightness;
					newb = b * distbrightness / 100. * ff->getBlueBrightness() * _brightness / 100.; // * _brightness;
			...
			for(int i = 0; i < SPARKLEMAX; i++)
			{
				...
				if(sparkle[2] > 0)
				{
					...
					//if((_brightness / 100. * .9) >= ff->getBrightness())
					...
					newr = ff->getSparkleRed() * ff->getRedBrightness() * bmode/(SPARKLECYCLE/2) * _brightness / 100.;
					newg = ff->getSparkleGreen() * ff->getGreenBrightness() * bmode/(SPARKLECYCLE/2) * _brightness / 100.;
					newb = ff->getSparkleBlue() * ff->getBlueBrightness() * bmode/(SPARKLECYCLE/2) * _brightness / 100.;
